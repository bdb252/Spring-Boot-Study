...
public class MainController {
	...
	/*
	방법1 : application.properties에 설정된 값은 @Value로 별도의 설정없이도 읽어올 수 있다.
		 읽어온 값을 멤버변수에 저장한다. */
	@Value("${test.key1}")
	...
	public String link1(Model model, @Value("${test.key3}") List<String> testKey3) {
		/*
		방법2 : 매개변수로 선언한 변수에 @Value를 부착한다. 이 경우 해당 메서드의 지역만 사용할 수 있다.
		 */
	...
	/*
	PropertyConfig 클래스에서 myprops라는 이름으로 빈을 생성했으므로 아래와 같이 
	값을 읽을 수 있다. 만약 이름을 지정하지 않으면 메서드명으로 생성된다.
	 */
	@Value("#{myprops['my.id']}")
	private String myId;
